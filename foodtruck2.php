...
$salad = new listitem("Salad","Romaine lettuce with cheese, tomato, and crutons",4.99,0);
$brownie = new listitem("Brownies","Gooey and chewy chocolate brownies",3.99,0);
$pretzel = new listitem("Pretzels","Whole wheat and healthy", 1.99, 0);  /* on1 change to an additional item */
...
{//data submitted
    ...
    $brownie->quantity = $_POST[$brownie->name];
    
    //var for number of salads user selected
    $salad->quantity = $_POST[$salad->name];
    $pretzel->quantity = $_POST[$pretzel->name];
    ...
    $subTotal = ($numSlices * $pizza->price) + ($brownie->total()) + ($salad->total() + ($pretzel->total()));
    ...
    /*
    if ($brownie->quantity != 0) {
        echo $brownie->quantity . ' side brownies<br/>';
    }
    */
    $brownie->printit();
    ...
    */
    ...
    echo '<br/>Sales Tax: $' . number_format(($subTotal * $salesTax),2);
    echo '<br/>The total for the order is $' .  number_format($total,2);
